    ...
    $("#registerForm").submit(function(){
                if( $("#registerForm").validationEngine('validate') == true){
                    ...
                    $.ajax({
                        ...
                        method:"post",
                        beforeSend:function(){
                            h = "<img src='images/loader.gif'> &nbsp;&nbsp;<span style='p'>  Signing you up .Please Wait....</span>";
                            $("#statusRegMsg").html(h);
                            $("#statusRegMsg").show('slow')
                        ...
                        complete:function(response){
                            ...
                            if(data.status=="success"){
                               $("#statusRegMsg").html("<span> You are now registerd  </span>")
                               $("#statusRegMsg").show('slow')
                               setTimeout(function(){ $("#registerModal").modal("hide"); $("#loginModal").modal("show"); }, 3000);
                               ...
                               $('#regemail').validationEngine('showPrompt', 'Email Already exist', 'error', true);  
                                ...
                                $("#statusRegMsg").html("<span> "+data.message+" </span>")
                                $("#statusRegMsg").show('slow')
                            }
                        }
                    });
        ...
        $("#loginBtn").click(function(){
            $("#loginForm").submit(function(){
                if( $("#loginForm").validationEngine('validate') == true){
                    //console.log($("#loginForm").attr("action"));
                    email = $("email");
                    passwd = $("pwd");
                    $.ajax({
                        ...
                        method:"post",
                        beforeSend:function(){
                            h = "<img src='images/loader.gif'> &nbsp;&nbsp;<span style='p'>  Signing In .Please Wait....</span>";
                            $("#statusMsg").html(h);
                            $("#statusMsg").show('slow')
                        ...
                        complete:function(response){
                            ...
                            if(data.status=="success"){
                                ...
                                $("#statusMsg").css('color',"#ff1a1a");
                                $("#statusMsg").html("<span> "+data.message+" </span>")
                                $("#statusMsg").show('slow')
                            }
                        }
                    });
